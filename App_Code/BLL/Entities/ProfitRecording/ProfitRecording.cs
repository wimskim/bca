...
namespace CryptoTrader.BLL
{
    ...
    public class ProfitRecording : BLL.BLLBaseObject
    {
        ...
        public decimal ExchangeAsk
        {
            get
            {
                if (_profitPerc > -10)
                    ...
                    return (_lunoBid - (_lunoBid * (1/10))); // if bitlish gives crazy price the return luno price -10%
            }
        ...
        private static void GenerateBTCProfitRecordings()
		{
            ...
            try
            {
                profPerc = GetProfit(bitlishUSD.ask.price, luno.bid.price, usdzar, 0.4M, 4.5M, 2000);
            ...
            try
            {
                profPerc = GetProfit(bitlishEUR.ask.price, luno.bid.price, eurzar, 0.4M, 4.5M, 1500);
                pf = new ProfitRecording();
                pf.Exchange = enExchange.Bitlish;
